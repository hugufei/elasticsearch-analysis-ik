...
public class SimpleSynonymMap {
	...
	private final SynonymConfiguration configuration;

	public SimpleSynonymMap(SynonymConfiguration cfg) {
		...
		if (sides.length > 1) { // explicit mapping
			...
			if (configuration.isExpand()) {
				...
				for (int i = 0; i < inputList.size(); i++) {
					addToRuleMap(inputList.get(i), inputList.get(0), false);
				}
			}
		}
	}
	...
	public List<String> getSynonymWords(String input, boolean isRewrite) {
		Map<String, List<String>> ruleMap = isRewrite ? this.rewriteRuleMap : this.sameRuleMap;
